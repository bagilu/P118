# P118 先民地圖 V0.4.2.1（年代資料刻度修正版）

本版將主要呈現方式由推定文化多邊形改為考古遺址點位。網站可直接放在 GitHub Pages，不需要 Node.js、npm 或編譯程序；Repository 根目錄直接包含 `index.html`。

## V0.4.2.1 重點

- 從穩定版V0.4.1重新製作，不沿用V0.4.2的介面程式。
- TIME／年代改為同一列的小型標題，年代數字與留白同步縮小。
- 自動播放按鈕只顯示往古代、暫停、往現代符號，播放程式維持V0.4.1原邏輯。
- 原操作說明改由右上方低調的小型「？」按鈕開啟。
- 年代拉桿下方新增細刻度；考古資料、全球事件及重合年代使用不同顏色。
- 新增介面元素均有缺漏保護，避免GitHub Pages快取版本不同步時中斷播放。
- CSS與JavaScript網址加入版本字串，降低GitHub Pages沿用舊快取的機會。
- 新增 `Database/13_AddXuntangpuCulture.sql`。
...
全新安裝請在 Supabase SQL Editor 依序執行 `Database/01_CreateTables.sql` 至 `13_AddXuntangpuCulture.sql`。

已完成V0.4.1者可直接替換前端檔案；若尚未加入訊塘埔文化，再執行 `Database/13_AddXuntangpuCulture.sql`。
