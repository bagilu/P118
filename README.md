# P118 先民地圖 V0.4.2（年代資料刻度）

本版將主要呈現方式由推定文化多邊形改為考古遺址點位。網站可直接放在 GitHub Pages，不需要 Node.js、npm 或編譯程序；Repository 根目錄直接包含 `index.html`。

## V0.4.2 重點

- 年代拉桿上方新增資料刻度，使用與滑桿相同的最早年、最晚年及線性比例定位。
- 考古資料、全球事件及兩者重合的年代使用不同深淺刻度。
- 抵達資料年代時，對應刻度會以橘色強調。
- 縮小 TIME／年代標題及年代區上下留白。
- 自動播放按鈕只保留往古代、暫停、往現代符號，完整名稱保留於輔助標籤。
- 操作說明改由「？」按鈕開啟，不再長期占用側欄版面。
- 新增 `Database/13_AddXuntangpuCulture.sql`，建立訊塘埔文化與第一批8處遺址關聯。
...
已完成V0.4.1者，可直接替換網站前端檔案。若要顯示訊塘埔文化，請另執行 `Database/13_AddXuntangpuCulture.sql`；該檔只操作P118物件。
